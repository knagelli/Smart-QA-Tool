...
# Hard ceiling on the actual number of test cases a trial generation run may
# produce. The word-count guard above is only a proxy for document size - a
# short but dense requirements list can still slip past it and, left
# unbounded, generate far more than 10 test cases in one call (real cost
# exposure, since output tokens are the bigger cost driver). This value is
# passed to the model as an explicit instruction (see qa_engine.run_qa_analysis's
# max_test_cases parameter) AND enforced again by truncating the result in
# main.py, so a model that ignores the instruction still can't produce an
# unbounded/uncapped trial report.
TRIAL_MAX_TEST_CASES = int(os.environ.get("TRIAL_MAX_TEST_CASES", "10"))
