...
async def _run_execution_batch_impl(
    ...
    def _push_status(current_tc_id=None, current_title=None):
        # Reported in the client's original selection order so "N of M
        # done" and the current-case indicator make sense to the client
        # even though creates:<type> cases are internally reordered first.
        results_so_far = [
            {"tc_id": t, "title": results_by_tc[t]["title"], "verdict": results_by_tc[t]["verdict"]}
            for t in selected_ids if t in results_by_tc
        ]
        exec_status.write_status(exec_dir, {
            "state": "running",
            "total": total,
            "completed": len(results_so_far),
            "current_tc_id": current_tc_id,
    ...
    for tc_id in scheduled_ids:
        ...
        _push_status(current_tc_id=tc_id, current_title=tc.get("title", ""))
    ...
    exec_status.write_status(exec_dir, {
        ...
        "current_title": None,
        "results_so_far": [
            {"tc_id": r["tc_id"], "title": r["title"], "verdict": r["verdict"]} for r in results
        ],
        "result_page": {
...
async def execute_status_json(run_id: str, exec_id: str, token: str = ""):
    ...
    return {
        ...
        "current_tc_id": status.get("current_tc_id"),
        "current_title": status.get("current_title"),
        "results_so_far": status.get("results_so_far", []),
    }
