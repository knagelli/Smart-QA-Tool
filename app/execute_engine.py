...
def execute_test_case(
    *,
    ...
    try:
        ...
        # TEMPORARY DIAGNOSTIC round 2 (remove once root cause of the
        # 2026-09-14 post-asyncio.to_thread BLOCKED issue is confirmed):
        # surface str(e) and a traceback snippet instead of only the
        # exception class name.
        import traceback as _tb
        tb_snip = "".join(_tb.format_exception(type(e), e, e.__traceback__))[-1500:]
        raise ExecutionError(f"DIAGNOSTIC2 {type(e).__name__}: {e}\n---tb---\n{tb_snip}") from e
