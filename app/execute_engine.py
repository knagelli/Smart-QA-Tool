    ...
    try:
        with sync_playwright() as p:
            try:
                ...
                # TEMPORARY DIAGNOSTIC (remove once root cause of the 2026-09-13
                # BLOCKED-with-no-steps issue is confirmed): surface the real
                # launch-time error instead of just the exception class name.
                raise ExecutionError(f"DIAGNOSTIC browser-launch failure: {type(e).__name__}: {e}"[:500]) from e
            try:
                ...
                for tu in tool_uses:
                    ...
                    try:
                        if name == "get_snapshot":
                            ...
                            if rl is not None:
                                rl.event("step", {"step": step_num, "action": "get_snapshot", "target": page.url, "result": "ok"})
                            ...
                            if rl is not None:
                                rl.event("step", {"step": step_num, "action": "finish_test", "verdict": verdict, "notes": notes})
                        else:
        ...
        # TEMPORARY DIAGNOSTIC (remove once root cause of the 2026-09-13
        # BLOCKED-with-no-steps issue is confirmed): include str(e), truncated,
        # instead of only the exception class name.
        raise ExecutionError(f"DIAGNOSTIC browser automation failed: {type(e).__name__}: {e}"[:500]) from e
